...
unless you explicitely tell it not to) deserialize the data into a Perl
...
Views in a Leyland application reside in the C<views/> directory, located
in the top level of the application. In this directory, you might have
files like "C<article.html>", "C<comment.html>", etc.
...
When a controller renders C<index.html> for output, C<index.html> will
...
You can items to it whenever you wish:
...
an array refernce with the following structure:
...
otherwise Leyland won't be able to serialize the structures. If a route
of your applications returns a blessed object, Leyland will go renegade
and use L<Data::Dumper> to serialize it.

The same will happen when you define that your route returns a format that
Leyland doesn't natively support, like YAML for example. Since Leyland
doesn't serialize structures to this format automatically, you are expected
to do so yourself. If you don't, however, Leyland will just use L<Data::Dumper>.
