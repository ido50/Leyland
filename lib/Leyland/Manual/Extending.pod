...
context class. Since it is L<Moose> based, you can create a class that
...
For example, our application can have the following class:
	...
	has 'db' => (is => 'ro', isa => 'SomeDatabaseClass', writer => '_set_db');
...
instance of our class in C<app.psgi>:
	...
	my $myapp = MyApp->new(config => $config, context_class => 'MyApp::Context');
...
Leyland also gives classes that extend Leyland::Context define a method
...
Of course, be careful if you decide to modify any of <Leyland::Context>'s
methods.

=head1 EXTENDING LEYLAND

If you find that Leyland lacks support for your preferred CPAN modules,
such as logging modules are template engines, you are more than welcome
to create these classes by yourself. Logging classes are expected to be
Moose classes that consume the L<Leyland::Logger> role. Refer to Leyland::Logger's
documentation for information on how to create a logging class. You can
also take a look on existing logging classes like L<Leyland::Logger::LogHandler>.

The same goes for view classes. These are expected to consume L<Leyland::View>,
which describes how view classes should be created. Look at L<Leyland::Tenjin>
for an example.

If you want to go even further, since Leyland is entirely Moose based,
and you're crazy enough, you can use Moose tricks and modifications to
...
by simply submitting suggestions and bug reports. Refer to <Leyland/"BUGS">
and <Leyland/"SUPPORT"> for more information on submitting bug reports.
