...
Usuaully, when everything is okay, your application will return a "200 OK"
...
unless you also pass C<< use_layouts => 0 >> to C<< $c->exception() >>.
...
with the URI to redirect to:
	...
	$c->exception({ code => 303, error => "Please see the provide URI", location => $c->uri_for('/something') });
