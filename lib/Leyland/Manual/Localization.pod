...
reached because language negotiation is quite impossibly, for two main
reasons:

=over

=item 1. In my experience, clients almost never send their preferred language as the HTTP
...
install on their PCs with an interface of their native language. We are talking about
...
in C<app.psgi>. This options should hold the path to a directory in which
...
in this directory, and store a L<Locale::Wolowitz> object as the "loc"
attribute of the context object. Then, you can use it like you would
any Locale::Wolowitz object. For example:
...
And that's pretty much it. Note that Leyland does not (and can't) localize
