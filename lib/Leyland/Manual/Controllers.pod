...
however, that when havin two or more routes with the exact same declaration in two
...
infinitely, Leyland won't care.

A controller class is a L<Moose> class that consumes the L<Leyland::Controller> role.
...
path exists with the exact request method, and another routes matches the
...
C<text/html> requests (and requests that don't have a content type). The same
goes for "returns". C<POST> and C<PUT> routes also automatically accept the
...
from the beginning and/or to the end), like C<get 'posts'> for example,
as you're likely to get the wrong results.
...
It expects a path to forward to, and possibly a list of arguments to pass
to the route:
...
the request, but instead will only find the first route that matches the
provided path and invoke it. So, theoretically, a POST route can forward
to a GET route, Leyland won't care. At times, however, multiple routes,
all accepting different HTTP methods, can match the same path. This can
make it difficult to know which route is gonna be forwarded to, and you
probably want to forward to a specific route. In this case, Leyland allows
you to define the route's HTTP method, like so:

	$c->forward('GET:/posts', $post->id);

	get '^/posts$' is 'internal' {
		my $post_id = shift;
		# do stuff
	}

So what you need to do is prepend the HTTP method (in it's proper capitalization)
and a colon to the path.
...
will be serialized and returned to the client. When internally forwarded
to, however, the forwarding route will not get the serialized data, but
...
You should keep that in mind as it can get confusing.
...
the "res" attribute of the context object):
...
to set a redirect, and still continue handling the request and/or performing
