...
=item * get: for GET requests. If you want your app to really be RESTful,
get routes should be "safe", in that they should not change the state of
the application. GET requests are as they're named: requests to I<get>
data, not create/modify/delete data. Your clients need to rest assured
application data will not be tampered as a result of GET requests.

=item * put: for PUT requests.

=item * post: for POST requests.

=item * del: for DELETE requests.
