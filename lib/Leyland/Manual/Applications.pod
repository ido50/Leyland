...
application (note that directories marked "(optional)" are pure suggestions
or examples, and are not integral to a Leyland application, which can perfectly live
without them):
...
A minimal controller will look something like this:
	...
	package Dribblist::Controller::Root;
...
handled by Leyland directly. Instead, Leyland relies on L<Plack::Middleware::Static>
to server those files. More information about static files in L<Leyland::Manual::StaticFiles>.
...
or whatever. Views and templates are generated by Leyland's view classes
, and are returned to the clients by your controllers. A view is written
...
L<Leyland::View::Tenjin> will be available for your app.

=item * "locales": takes a path (can be relative) to a directory containing
your application's L<Locale::Wolowitz> localization files. If not provided,
your app is assumed mono-lingual.
...
if configuration options are available for the selected environment are
available, they will also be used, and will even take precedence over
...
Plus, every environment has a its own "database_host" value, which is
...
listening on port 5000. Read L<Leyland::Manual::Deployment> and L<plackup>
